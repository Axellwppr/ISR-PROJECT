...
class LeggedRobot(BaseTask):
    ...
    def setup_kin_info(self):
        if self.cfg.train.distill:
            ...
            self.kin_dict.update(
                {
                    "gt_action": torch.zeros([self.num_envs, self.num_actions]).to(
                        self.device
                    ),
                }
    ...
    def load_expert(self):
        ...
        net_key_name = "actor"
        loading_keys = [
            k for k in model_dict[model_key].keys() if k.startswith(net_key_name)
        ]
        ...
        if self.cfg.train_velocity_estimation:
            ...
            if self.common_step_counter % 5000 == 0:
                ...
                load_path = os.path.join(
                    LEGGED_GYM_ROOT_DIR,
                    model_dir,
                    "velocity_estimator_" + str(self.common_step_counter) + ".pt",
                )
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, base_id, :], self._rigid_body_ang_vel[:, base_id, :]
        )

        self.projected_gravity[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, base_id, :], self.gravity_vec
        )
    ...
    def check_termination(self):
        ...
        # print("Terminated by contact force: ", torch.sum(self.reset_buf).item())
        ...
        if self.cfg.asset.terminate_by_gravity:
            ...
            # print(self.projected_gravity[:, z_axis])
            self.reset_buf |= torch.any(
                torch.abs(self.projected_gravity[:, z_axis])
                < 0.7,
                dim=1,
            )
            # self.reset_buf |= torch.any(
            ...
            #     dim=1,
            # )
            # # print("Terminated by gravity x: ", torch.sum(self.reset_buf).item())

            # # Termination for gravity in y-direction
            # self.reset_buf |= torch.any(
            #     torch.abs(self.projected_gravity[:, 2:3])
            #     > self.cfg.asset.termination_scales.gravity_y,
            #     dim=1,
            # )
            # print("Terminated by gravity y: ", torch.sum(self.reset_buf).item())
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.terminate_by_ref_motion_distance:
                ...
                if self.cfg.asset.local_upper_reward:
                    ...
                    ref_body_pos[:, upper_body_ids] -= diff

                if self.cfg.env.test or self.cfg.env.im_eval:
                    reset_buf_teleop = torch.any(
                        torch.norm(self._rigid_body_pos - ref_body_pos, dim=-1).mean(
                            dim=-1, keepdim=True
                        )
                        > termination_distance,
                        dim=-1,
        ...
        # print("Terminated by time out: ", torch.sum(self.reset_buf).item())
        # if self.cfg.motion.teleop:
    ...
    def update_freeze_ref(self, motion_res, index):
        ...
        self.freeze_motion_res["body_ang_vel"][index] = motion_res["body_ang_vel"][
            index
        ]  # [num_envs, num_markers, 3]
    ...
    def compute_self_and_task_obs(
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.future_tracks:
                ...
                motion_times_steps = (
                    (self.episode_length_buf[self.motion_ids, None] + 1) * self.dt
                    + time_internals
                ...
                motion_res = self._get_state_from_motionlib_cache_trimesh(
                    env_ids_steps,
                    motion_times_steps,
                    offset=offset.repeat_interleave(time_steps, dim=0).view(-1, 3),
                )
            else:
                B = self.motion_ids.shape[0]
                motion_times = (
            ...
            if self.cfg.domain_rand.motion_package_loss:
                ...
                ref_body_pos_extend = self.freeze_motion_res["rg_pos"]
                ...
                ref_body_rot_extend = motion_res["rb_rot"]  # [num_envs, num_markers, 4]
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == "v1":
                with torch.no_grad():
                    ...
                    obs = torch.cat(
                        [self_obs, task_obs, self.projected_gravity, self.actions],
                        dim=-1,
                    )
                ...
                if (
                    ...
                    if self.cfg.asset.zero_out_far_change_obs:
                        ...
                        if self.cfg.motion.future_tracks:
                            ...
                            # print(ref_rb_pos_subset[zeros_set_future, :2])
                            root_vel_ = (
                                root_vel[zeros_subset].unsqueeze(1).repeat(1, 3, 1)
                            )
                            ...
                            dis_new = (
                                (
                                    ref_rb_pos_subset[vector_zero_subset_future, 2]
                            ...
                            ref_rb_pos_subset[zeros_subset, :2] = body_pos_subset[
                                zeros_subset, :2
                            ]  # two hands\
                ...
                self.cfg.motion.teleop_obs_version == "v-teleop-extend-vr-max-nolinvel"
            ):
                body_pos = self._rigid_body_pos
                body_rot = self._rigid_body_rot
                body_vel = self._rigid_body_vel
                ...
                body_vel_extend = body_vel.clone()
                body_vel_subset = body_vel_extend[:, self._track_bodies_extend_id, :]

                body_ang_vel_extend = body_ang_vel.clone()
            ...
            if self.cfg.motion.teleop:
                if self.cfg.motion.realtime_vr_keypoints:
                    ...
                    for pos_id, pos_joint in enumerate(
                        ...
                        sphere_pose = gymapi.Transform(
                            gymapi.Vec3(pos_joint[0], pos_joint[1], pos_joint[2]),
                            r=None,
                        )
                        gymutil.draw_lines(
                            sphere_geom_marker,
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.domain_rand.motion_package_loss and (
            self.common_step_counter % self.cfg.domain_rand.package_loss_interval == 0
        ):
        ...
        if self.cfg.motion.teleop and (
            ...
            % self.cfg.motion.resample_motions_for_envs_interval_s
            == 0
        ):
    ...
    def _reset_dofs(self, env_ids):
        ...
        if self.cfg.motion.teleop:
            ...
            # print("reset dof", self.dof_pos[env_ids])
        ...
        pos_target = self.dof_pos.clone()
        self.gym.set_dof_position_target_tensor_indexed(
            self.sim,
            gymtorch.unwrap_tensor(pos_target),
            gymtorch.unwrap_tensor(env_ids_int32),
            len(env_ids_int32),
        )
        # self.gym.set_dof_target_tensor_indexed(
        #     self.sim,
        #     gymtorch.unwrap_tensor(self.dof_pos),
        #     gymtorch.unwrap_tensor(env_ids_int32),
        #     len(env_ids_int32),
        # )
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:  # trimesh
            if self.cfg.motion.teleop:
                ...
                # motion_res["root_rot"][:, 0] = 0.0
                # motion_res["root_rot"][:, 1] = 0.0
                # motion_res["root_rot"][:, 2] = 0.71
                # motion_res["root_rot"][:, 3] = 0.71
                # breakpoint()
    ...
    def _update_teleop_curriculum(self, env_ids):
        ...
        if self.cfg.motion.teleop:
            ...
            move_up = (
                episode_length_buf > self.cfg.motion.teleop_level_up_episode_length
            )
            move_down = (
                episode_length_buf < self.cfg.motion.teleop_level_down_episode_length
            )
        else:
            raise NotImplementedError
        # import ipdb; ipdb.set_trace()
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.teleop_obs_version == "v1":
                ...
                noise_vec[0:body_pos_end_idx] = (
                    noise_scales.body_pos * noise_level * self.obs_scales.body_pos
                )  # [1: 58]
                ...
                noise_vec[body_pos_end_idx:body_rot_end_idx] = (
                    noise_scales.body_rot * noise_level * self.obs_scales.body_rot
                )  # [58: 178]
                ...
                noise_vec[
                    ...
                    noise_scales.ref_body_rot * noise_level * self.obs_scales.body_rot
                )  # [282:402]
                ...
                noise_vec[
                    ...
                    noise_scales.ref_lin_vel * noise_level * self.obs_scales.lin_vel
                )  # [402:405]

                diff_local_ang_vel_end_idx = diff_local_lin_vel_end_idx + 3  # 3dim
                noise_vec[diff_local_lin_vel_end_idx:diff_local_ang_vel_end_idx] = (
                    noise_scales.ref_ang_vel * noise_level * self.obs_scales.ang_vel
                )  # [405:408]
                ...
                diff_local_ref_body_rot_root_end_idx = (
                    diff_local_ref_body_pos_root_end_idx + self.num_actions * 6 + 6
                )  # 6x20 -> 120dim
                noise_vec[
                ...
                noise_vec[diff_local_ref_body_rot_root_end_idx:diff_dof_pos_end_idx] = (
                    noise_scales.ref_dof_pos * noise_level * self.obs_scales.dof_pos
                )  # [588:607]
                ...
                noise_vec[diff_dof_pos_end_idx:diff_dof_vel_end_idx] = (
                    noise_scales.ref_dof_vel * noise_level * self.obs_scales.dof_vel
                )  # [607:626]
                ...
                noise_vec[
                    ...
                    + 9 : 2 * self.num_dof
                    + 9
                    + (len(self.cfg.motion.teleop_selected_keypoints_names) + 2) * 3
                ] = (noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos)
            elif self.cfg.motion.teleop_obs_version == "v-teleop-extend-nolinvel":
                ...
                noise_vec[
                    ...
                    + 9 : 2 * self.num_dof
                    + 9
                    + (len(self.cfg.motion.teleop_selected_keypoints_names) + 2) * 3 * 3
                ] = (noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos)
                ...
                noise_vec[0 : (max_num_bodies - 1) * 3] = (
                    noise_scales.body_pos * noise_level * self.obs_scales.dof_pos
                )
                curr_obs_len += (max_num_bodies - 1) * 3

                # body_rot
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 6] = (
                    noise_scales.body_rot * noise_level * self.obs_scales.dof_vel
                )
                curr_obs_len += max_num_bodies * 6

                # body vel
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 3] = (
                    noise_scales.body_lin_vel * noise_level * self.obs_scales.lin_vel
                )
                curr_obs_len += max_num_bodies * 3

                # body ang vel
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 3] = (
                    noise_scales.body_ang_vel * noise_level * self.obs_scales.ang_vel
                )
                ...
                # ref body_rot diff
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 6] = (
                    noise_scales.ref_body_rot * noise_level * self.obs_scales.body_pos
                )
                curr_obs_len += max_num_bodies * 6

                # ref lin vel diff
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 3] = (
                    noise_scales.ref_lin_vel * noise_level * self.obs_scales.body_pos
                )
                curr_obs_len += max_num_bodies * 3

                # ref ang vel diff
                noise_vec[curr_obs_len : curr_obs_len + max_num_bodies * 3] = (
                    noise_scales.ref_ang_vel * noise_level * self.obs_scales.body_pos
                )
                curr_obs_len += max_num_bodies * 3
                ...
                # base ang vel
                noise_vec[2 * self.num_dof + 3 - 3 : 2 * self.num_dof + 6 - 3] = (
                    noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                )
                # base gravity
                noise_vec[2 * self.num_dof + 6 - 3 : 2 * self.num_dof + 9 - 3] = (
                    noise_scales.gravity * noise_level
                )
                # ref dof pos
                noise_vec[
    ...
    def _episodic_domain_randomization(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_pd_gain:
            ...
            if (
                self.cfg.motion.curriculum and self.cfg.motion.kpkd_by_curriculum
        ...
        if self.cfg.domain_rand.randomize_rfi_lim:
            ...
            if self.cfg.motion.curriculum and self.cfg.motion.rfi_by_curriculum:
                rfi_lim_scale_offset_from_lowerlimit = (
                    self._rfi_lim_scale[env_ids] - self.cfg.domain_rand.rfi_lim_range[0]
                )
                self._rfi_lim_scale[env_ids] -= rfi_lim_scale_offset_from_lowerlimit * (
                    1 - self.teleop_levels[env_ids].unsqueeze(-1) / 10.0
                )
    ...
    def _init_buffers(self):
        ...
        if self.cfg.motion.teleop:
            ...
            self.marker_coords = torch.zeros(
                self.num_envs,
                (self.num_dofs + 1) * self.cfg.motion.num_traj_samples,
                3,
                dtype=torch.float,
                device=self.device,
                requires_grad=False,
    ...
    def _create_envs(self):
        ...
        # breakpoint()
        # dof_props_asset["stiffness"] = 200.0
        # dof_props_asset["damping"] = 5.0
    ...
    def _get_rigid_body_pos(self, body_name):
        ...
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        return gymtorch.wrap_tensor(self.gym.acquire_rigid_body_state_tensor(self.sim))[
    ...
    def feet_distance(self):
        assert False, "reward not implemented for XBot"
    ...
    def _reward_orientation(self):
        ...
        return torch.sum(torch.square(self.projected_gravity[:, xy_axis]), dim=1)
    ...
    def _reward_lower_action_rate(self):
        # Penalize changes in actions
        return torch.sum(
            torch.square(self.last_actions[:, lower_body_ids] - self.actions[:, lower_body_ids]), dim=1
        )

    def _reward_upper_action_rate(self):
        # Penalize changes in actions
        return torch.sum(
            torch.square(self.last_actions[:, upper_body_ids] - self.actions[:, upper_body_ids]), dim=1
        )
    ...
    def _reward_freeze_upper_body(self):
        assert False, "reward not implemented for XBot"
        return torch.mean(
            torch.square(self.dof_pos[:, 10:] - self.default_dof_pos[:, 10:]), dim=1
        )
    ...
    def _reward_teleop_joint_position_lower(self):
        ...
        diff_dof_pos = diff_dof_pos[:, lower_body_ids]  # lower, not including torso
    ...
    def _reward_teleop_joint_position_upper(self):
        ...
        diff_dof_pos = ref_dof_pos - dof_pos
        diff_dof_pos = diff_dof_pos[:, upper_body_ids]  # upper
    ...
    def _reward_teleop_body_position_extend(self):
        ...
        if self.cfg.asset.local_upper_reward:
            ...
            ref_body_pos_extend[:, upper_body_ids] -= diff

        body_pos_extend = body_pos.clone()

        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_lower = diff_global_body_pos[:, lower_body_ids]
        diff_global_body_pos_upper = diff_global_body_pos[:, upper_body_ids]
        ...
        r_body_pos_lower = torch.exp(
            -diff_body_pos_dist_lower / self.cfg.rewards.teleop_body_pos_lowerbody_sigma
        )
        r_body_pos_upper = torch.exp(
            -diff_body_pos_dist_upper / self.cfg.rewards.teleop_body_pos_upperbody_sigma
        )
    ...
    def _reward_teleop_body_position_extend_upper_0dot5sigma(self):
        ...
        diff_body_pos_dist_upper = diff_body_pos_dist_upper
        r_body_pos_upper = torch.exp(
            -diff_body_pos_dist_upper / self.cfg.rewards.teleop_body_pos_0dot5sigma
        )
    ...
    def _reward_teleop_body_position_vr_3keypoints(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_vr_3keypoints = diff_global_body_pos[:, vr3keypoint_ids]
    ...
    def _reward_teleop_body_rotation_lower(self):
        ...
        diff_global_body_angle = torch_utils.quat_to_angle_axis(diff_global_body_rot)[0]
        diff_global_body_angle = diff_global_body_angle[:, lower_body_ids]  # lower
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(
            -diff_global_body_angle_dist / self.cfg.rewards.teleop_body_rot_sigma
        )
    ...
    def _reward_teleop_body_rotation_upper(self):
        ...
        diff_global_body_angle = torch_utils.quat_to_angle_axis(diff_global_body_rot)[0]
        diff_global_body_angle = diff_global_body_angle[:, upper_body_ids]  # upper
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(
            -diff_global_body_angle_dist / self.cfg.rewards.teleop_body_rot_sigma
        )
    ...
    def _reward_teleop_body_vel_lower(self):
        ...
        diff_global_vel = ref_body_vel - body_vel
        diff_global_vel = diff_global_vel[:, lower_body_ids]
        diff_global_vel_dist = (diff_global_vel**2).mean(dim=-1).mean(dim=-1)
    ...
    def _reward_teleop_body_vel_upper(self):
        ...
        diff_global_vel = ref_body_vel - body_vel
        diff_global_vel = diff_global_vel[:, upper_body_ids]
        diff_global_vel_dist = (diff_global_vel**2).mean(dim=-1).mean(dim=-1)
    ...
    def _reward_teleop_selected_body_vel(self):
        ...
        diff_global_vel_dist = (
            (diff_global_vel[:, track_vel_body_indices] ** 2).mean(dim=-1).mean(dim=-1)
        )
        r_vel = torch.exp(
            -diff_global_vel_dist / self.cfg.rewards.teleop_body_vel_sigma
        )
    ...
    def _reward_teleop_body_ang_vel(self):
        ...
        r_ang_vel = torch.exp(
            -diff_global_ang_vel_dist / self.cfg.rewards.teleop_body_ang_vel_sigma
        )
        return r_ang_vel
    ...
    def _reward_teleop_body_ang_vel_lower(self):
        ...
        diff_global_ang_vel = ref_body_ang_vel - body_ang_vel
        diff_global_ang_vel = diff_global_ang_vel[:, lower_body_ids]  # lower
        diff_global_ang_vel_dist = (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
        r_ang_vel = torch.exp(
            -diff_global_ang_vel_dist / self.cfg.rewards.teleop_body_ang_vel_sigma
        )
    ...
    def _reward_teleop_body_ang_vel_upper(self):
        ...
        diff_global_ang_vel = ref_body_ang_vel - body_ang_vel
        diff_global_ang_vel = diff_global_ang_vel[:, upper_body_ids]  # upper
        diff_global_ang_vel_dist = (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
        r_ang_vel = torch.exp(
            -diff_global_ang_vel_dist / self.cfg.rewards.teleop_body_ang_vel_sigma
        )
    ...
    def _reward_feet_max_height_for_this_air(self):
        ...
        self.feet_air_max_height = torch.max(
            self.feet_air_max_height, self._rigid_body_pos[:, self.feet_indices, 2]
        )
    ...
    def _reward_move_or_not(self):
        should_move = torch.logical_or(
            torch.norm(self.commands[:, :2], dim=1) > 0.1, self.commands[:, 2] > 0.2
        )
    ...
    def _reward_freeze_arms(self):
        ...
        return torch.sum(
            torch.square(self.dof_pos[:, upper_body_ids] - self.default_dof_pos[:, upper_body_ids]), dim=1
...
def compute_imitation_observations_teleop_max(
    ...
    if ref_episodic_offset is not None:
        ...
        diff_local_body_pos_flat = (
            diff_local_body_pos_flat.view(B, 1, J, 3) + diff_global_body_pos_offset
        )
...
def compute_imitation_observations_teleop_max_heading(
    ...
    diff_global_body_rot = torch_utils.quat_mul(
        ...
        torch_utils.quat_conjugate(head_rot[:, None].repeat_interleave(time_steps, 1)),
    )
    diff_local_body_rot_flat = torch_utils.quat_mul(
        torch_utils.quat_mul(
            heading_inv_rot_expand.view(-1, 4), diff_global_body_rot.view(-1, 4)
