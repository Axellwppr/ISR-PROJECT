...
if __name__ == "__main__":
    ...
    data_dump = {}
    pbar = tqdm(amass.keys())
    # 打乱数据顺序
    # pbar = tqdm(np.random.permutation(list(amass.keys())))
    ...
    for data_key in pbar:
        breakpoint()
        amass_data = amass[data_key]
        if amass_data is None:
            continue

        skip = int(amass_data["fps"] // 30) if amass_data["fps"] >= 30 else 1
        trans = (
            torch.from_numpy(amass_data["root_trans_offset"][::skip]).float().to(device)
        )
        # print(trans)
        N = trans.shape[0]
        pose_aa_walk = (
            torch.from_numpy(
                np.concatenate(
                    (
                        amass_data["smpl_joints"][::skip, :66],
                        np.zeros((N, 6)),
                    ),
                    axis=-1,
                )
